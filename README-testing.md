...
`src/core/webhook.py` is built on **raw aiohttp** (`aiohttp.web.Application`), not
aiogram's `SimpleRequestHandler`. The `/webhook` POST route enforces the same
contract as aiogram: it requires the `X-Telegram-Bot-Api-Secret-Token` header to
match `Config.webhook_secret`. `tests/test_webhook.py` asserts: valid secret →
200/202, wrong/missing secret → 401/403, malformed JSON → 4xx/5xx. The secret is
read from `WEBHOOK_SECRET` in the environment (see `.env.example`).
...
- aiogram 3.x: routing/FSM tested via `Dispatcher.feed_update` / `feed_raw_update` +
  `AsyncMock` Bot session; `aiogram-tests` is NOT used as the sole foundation.
